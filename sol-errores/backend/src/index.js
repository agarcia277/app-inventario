...
app.use('/api/floorplan-categories', require('./routes/floorplan-categories'));
  ...
  try {
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS floorplan_categories (
        ...
        default_color VARCHAR(20) DEFAULT '#1e3a5f',
        default_w INTEGER DEFAULT 100,
        default_h INTEGER DEFAULT 100,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
      );
    ...
    // Poblar floorplan_categories por defecto si está vacío
    const { rows: catCount } = await client.query('SELECT COUNT(*) FROM floorplan_categories');
    if (parseInt(catCount[0].count) === 0) {
      await client.query(`
        INSERT INTO floorplan_categories (type, label, icon, default_color, default_w, default_h) VALUES
        ('room', 'Sala / Zona', 'Square', '#1e3a5f', 200, 150),
        ('desk', 'Mesa / Puesto', 'Move', '#1a3a2a', 100, 60),
        ('server_rack', 'Rack Servidor', 'Server', '#3b1f5e', 60, 100),
        ('printer', 'Impresora', 'Printer', '#3d2a0a', 70, 70),
        ('switch', 'Switch / Router', 'Wifi', '#1a3a2a', 80, 40),
        ('wall', 'Pared', 'Minus', '#374151', 200, 16),
        ('door', 'Puerta', 'DoorOpen', '#92400e', 60, 16),
        ('label', 'Etiqueta texto', 'Type', '#1f2937', 120, 36),
        ('asset', 'Activo IT', 'Server', '#1e3a5f', 80, 80)
      `);
    }
